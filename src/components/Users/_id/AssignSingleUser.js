...
export default function AssignSingleUser(props) {
  ...
  const SubmitData = (e) => {
    ...
    let assignList = [];
    assignList.push({
      user_id: sendData.users.user_id,
      quantity: sendData.quantity,
      delivery_type: sendData.delivery_type,
      product_type: sendData.product_type,
      price: sendData.price,
  ...
  function getSteps() {
    return ["Select Agent", "Select Product", "Select Quantity", "Select Delivery type"];
  }

  const handleData = (e, g, n) => {
    e && e.preventDefault();
    if (n === "agent") {
      setSenddata({ ...sendData, agent_id: g, users: props.currUser });
    }
  ...
  function getStepContent(step) {
    switch (step) {
      case 0:
        return (
          <>
            <SelectAgent access_token={userInfo.access_token} refreshAccessToken={userInfo.refreshAccessToken} handleData={handleData} />
          </>
        );
      ...
      //     return (
      //       <>
      //         <SelectUser handleData={handleData} refreshAccessToken={userInfo.refreshAccessToken} access_token={userInfo.access_token} />
      //       </>
      //     );
      case 1:
        return (
      ...
      case 3:
        return (
          <>
            <SelectDeliveryType handleData={handleData} refreshAccessToken={userInfo.refreshAccessToken} access_token={userInfo.access_token} />
          </>
        );
      default:
        return "unknown step";
          ...
          {activeStep === steps.length ? (
            ...
            <>
              ...
              <Button
                ...
                color="primary"
                onClick={(e) => (activeStep === steps.length - 1 ? SubmitData(e) : handleNext(e))}
              >
